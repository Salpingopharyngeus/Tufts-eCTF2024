...
#include "bcrypt.h"
#include "application_processor/crypt_blowfish/ow-crypt.h"
...
int main(void)
{
	...
	assert(ret == 0);
	// updated to use strncmp instead of strcmp
	printf("First hash check: %s\n", (strncmp(hash1, hash) == 0)?"OK":"FAIL");
	ret = bcrypt_hashpw(pass, hash2, hash);
	assert(ret == 0);
	// updated to use strncmp instead of strcmp
	printf("Second hash check: %s\n", (strncmp(hash2, hash) == 0)?"OK":"FAIL");

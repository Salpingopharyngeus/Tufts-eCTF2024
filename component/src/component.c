...
void secure_send(uint8_t *buffer, uint8_t len) {
    ...
    if (assigned_random_number == 0) {
        print_error(
            "Component attempting to initiate communication with AP first!\n");
        return ERROR_RETURN;
    }
    ...
    memcpy(temp_buffer + hash_position, hash_out,
           HASH_SIZE);                    // add authentication hash
...
void process_attest() {
    ...
    uint8_t EXACT_SIZE =
        attest_loc_size + attest_date_size + attest_cust_size + fixed_size;

    char attestation_data[ATTEST_SIZE]; // Assuming a sufficiently large buffer
                                        // size
    ...
    uint8_to_uint32(temp_buffer, sizeof(temp_buffer), uint32_temp,
                    sizeof(uint32_temp) / sizeof(uint32_t));
    ...
    // Encrypt contents of uint32_t representation of attestation data and store
    // result in uint32_t transmit buffer
    int aes_success =
        AES_encrypt(0, MXC_AES_128BITS, uint32_temp, uint32_transmit_buffer);
    ...
    uint32_to_uint8(uint32_transmit_buffer, num_elements, uint8_transmit_buffer,
                    uint8_buffer_size);

    // Include exact attestation data size in the transmit buffer
    size_t total_size =
        uint8_buffer_size +
        1; // where 4 bytes represents the exact attestation data size
    ...
    memcpy(mod_uint8_transmit_buffer + 1, uint8_transmit_buffer,
           uint8_buffer_size);
